...
function initButtonHover() {
  document
    .querySelectorAll(".btn-primary, .nav-cta, .project-link")
    .forEach((btn) => {
      hover(btn, () => {
        animate(
          btn,
          { y: -2, scale: 1.02 },
          { duration: DUR_MED, easing: EASE_OUT }
        );
        return () =>
          animate(
            btn,
  ...
  document
    .querySelectorAll(".btn-ghost, .project-repo, .video-link")
    .forEach((btn) => {
      hover(btn, () => {
...
function initAvatarHover() {
  ...
  hover(avatar, () => {
    ...
    return () =>
      animate(avatar, { scale: 1 }, { duration: DUR_SLOW, easing: EASE_OUT });
  });
}
...
function initSocialHover() {
  document.querySelectorAll(".social-links a").forEach((link) => {
    hover(link, () => {
      ...
      return () =>
        animate(link, { y: 0 }, { duration: DUR_MED, easing: EASE_OUT });
    });
  });
}
...
function initToggleAnimation() {
  ...
  btn.addEventListener("click", () => {
    animate(
      btn,
      { scale: [1, 0.85, 1], rotate: [0, 15, -5, 0] },
      {
        duration: 0.4,
        easing: EASE_OUT,
      }
...
function initPageTransitions() {
  ...
  if (bg) {
    animate(bg, { opacity: 0, scale: 1 }, { duration: 0 });
  }
  ...
  document.querySelectorAll("a").forEach((link) => {
    link.addEventListener("click", (e) => {
      ...
      if (href) {
        ...
        targetLink = Array.from(navLinks || []).find((l) => {
      ...
      window.__moveNavIndicator?.(targetLink);
...
function initNavIndicator() {
  ...
  // expose global để click handler dùng
  window.__moveNavIndicator = moveTo;
